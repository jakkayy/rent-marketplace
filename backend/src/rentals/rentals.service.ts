...
export class RentalsService {
  ...
  async findMyRentals(renterId: string) {
    return this.prisma.rental.findMany({
      where: { renterId },
      include: { product: { select: { id: true, name: true, images: true } }, shop: { select: { id: true, name: true } } },
  ...
  async findShopRentals(userId: string) {
    ...
    return this.prisma.rental.findMany({
      ...
      include: { product: { select: { id: true, name: true, images: true } }, renter: { select: { id: true, firstName: true, lastName: true, phone: true } } },
      orderBy: { createdAt: 'desc' },
    });
  }
  ...
  async cancelRental(userId: string, rentalId: string) {
    ...
    if (rental.status === RentalStatus.COMPLETED) throw new BadRequestException('Cannot cancel completed rental');
